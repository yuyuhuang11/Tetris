...
public class Square {
	...
	public boolean canRotate(int pivotRow, int pivotCol){
		int nextRow, nextCol;

		int currentRow = row;
		int currentCol = col;

		nextRow = pivotRow - pivotCol + currentCol;
		nextCol = pivotCol + pivotRow - currentRow;

		boolean rotate = true;

		// Each square must be inside the grid
		if (nextRow < 0 || nextRow > 19 || nextCol < 0 || nextCol > 9){
			rotate = false;
		}

		return rotate && (!grid.isSet(nextRow, nextCol));
	}


	public void rotate(int pivotRow, int pivotCol){

		int currentRow = row;
		int currentCol = col;

		if (canRotate(pivotRow, pivotCol)){
			row = pivotRow - pivotCol + currentCol;
			col = pivotCol + pivotRow - currentRow;
		}
	}
